...
function replaceNikkiLifetimeSummary(discordId, summary) {
  getDb().prepare(`
    INSERT OR REPLACE INTO nikki_lifetime_summary
      (discord_id, periodic_draw_num, permanent_draw_num, updated_at)
    VALUES (?, ?, ?, ?)
  `).run(discordId, summary.periodic_draw_num, summary.permanent_draw_num, Date.now());
}

function getNikkiLifetimeSummary(discordId) {
  return getDb().prepare(
    `SELECT periodic_draw_num, permanent_draw_num, updated_at
